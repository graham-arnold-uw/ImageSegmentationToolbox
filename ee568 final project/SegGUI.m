...
% SEGGUI MATLAB code for SegGUI.fig
...
% Last Modified by GUIDE v2.5 14-Mar-2017 17:49:43
...
function displayImage(mask)

global X;
global hAxes1;

output = ApplyMask(mask,X);
set(gcf, 'CurrentAxes', hAxes1);
imshow(output);
return
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    scaling = get(hSlider,'Value');
    mask = GreenThresh(X,scaling);
    displayImage(mask);
...
% handles    structure with handles and user data (see GUIDATA)
    global X;
    global mask;
    hSlider = findobj(gcf,'Tag','slider3');
    size = get(hSlider,'Value');
    size = floor(size);
    mask = FillCenter(mask,size);
    displayImage(mask);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    output = ApplyMask(mask,X);
    imwrite(output,'SegmentedImage.png','png');
